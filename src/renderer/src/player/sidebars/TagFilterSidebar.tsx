...
import { Box, Text, ScrollArea, UnstyledButton, Stack, Divider, Group } from '@mantine/core'
      ...
      >
        ...
        <Box
          style={{
            flex: 1,
            overflow: 'hidden',
            // 确保容器至少有 20px 底部间距方便双击，或者点击列表项之间的缝隙
            paddingBottom: '40px'
          }}
          onDoubleClick={(e) => {
            if (e.target === e.currentTarget || (e.target as HTMLElement).tagName === 'DIV') {
              const isClickOnCard =
                (e.target as HTMLElement).innerText.length > 0 && e.target !== e.currentTarget
              if (!isClickOnCard) {
            ...
            {orderedGroups.map((group, index) => {
              ...
              return (
                <Box key={group}>
                  {/* 如果颜色发生变化，插入一条分割线 */}
                  {isColorChanged && (
                    <Divider my={4} color="var(--mantine-color-dark-4)" style={{ opacity: 0.5 }} />
                  )}
        ...
        <Box style={{ flex: 1, minHeight: 0 }}>
          <ScrollArea h="100%" p="xs">
            <Box style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '8px' }}>
              {activeGroup &&
                tagsData[activeGroup]?.map((tag) => (
                  <TagCard
                    key={tag.id}
                    tag={tag}
                    onClick={() => setSelectedTagId(tag.id)}
                    borderColor="var(--mantine-color-blue-7)"
                    hoverBorderColor="var(--mantine-color-blue-4)"
                  />
                ))}
            </Box>
          </ScrollArea>
