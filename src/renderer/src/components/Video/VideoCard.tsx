...
import { Box, Image, ActionIcon, Group, Paper, Text, Stack, Portal } from '@mantine/core'
...
import { useFileActions } from '@renderer/hooks'
  ...
  return (
    <Box
      ...
      style={{
        ...
        overflow: 'hidden',
        // 选中状态边框变色
        border: isSelected ? '2px solid var(--mantine-color-blue-6)' : '2px solid #2C2E33',
        cursor: 'pointer',
        backgroundColor: '#000',
        aspectRatio: '16/9',
        // 选中时稍微增加阴影或缩放感
          ...
          >
            <Stack gap={1}>
              <MenuBtn
                ...
                label="打开文件夹"
                onClick={() => {
                  handleShowInExplorer(video.path)
                  setMenuPos(null)
                }}
              />
              <MenuBtn
...
function MenuBtn({ icon, label, onClick, color }: any) {
  return (
    ...
    >
      <Box style={{ color: color ? 'var(--mantine-color-red-5)' : '#A6A7AB', display: 'flex' }}>
        {icon}
      </Box>
      <Text size="sm" c={color || 'gray.3'}>
        {label}
      </Text>
    </Group>
  )
