...
export const usePlaylistStore = create<PlaylistState>((set, get) => ({
  ...
  next: async () => {
    ...
    switch (mode) {
      case 'liked':
        ...
        break
      case 'elite':
        queue = selectElitePaths(registry)
        break
      case 'search':
        queue = selectSearchPaths(registry, searchQuery)
        break
      case 'tag_filter':
  ...
  _internalUpdateHistory: (path: string) => {
    ...
    // 1. 构造新数组：去重并置顶
    const newHistory = [path, ...historyPaths.filter((p) => p !== path)].slice(0, 100) // 只保留最近100条
