...
function findBinaries() {
  ...
  const binPath =
    fs.existsSync(path.join(prodPath, 'ffmpeg.exe')) || fs.existsSync(path.join(prodPath, 'ffmpeg'))
      ? prodPath
      : devPath
...
export function setupFfmpeg() {
  if (fs.existsSync(ffmpegFullPath)) {
    ...
    log.error(
      `[FFmpegConfig] CRITICAL: ffmpeg binary not found at the configured path: ${ffmpegFullPath}`
    )
  }
  ...
  if (fs.existsSync(ffprobeFullPath)) {
    ...
    log.error(
      `[FFmpegConfig] CRITICAL: ffprobe binary not found at the configured path: ${ffprobeFullPath}`
    )
  }
}
