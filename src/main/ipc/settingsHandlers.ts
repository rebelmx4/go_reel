...
export function registerSettingsHandlers() {
  ...
  ipcMain.handle('get-path-overview', async () => {
    return {
      ...
      data_directory: path.join(process.cwd(), 'data/data'), // 数据存储目录是固定的
  ...
  ipcMain.handle(
    'save-key-bindings',
    async (_, newKeyBindings: PreferenceSettings['key_bindings']) => {
  ...
  ipcMain.handle(
    ...
    async (_, settingsToUpdate: Partial<PreferenceSettings>) => {
      try {
