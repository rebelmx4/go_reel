...
export abstract class BaseJsonManager<T> {
  ...
  constructor(
    fileName: string,
    defaultData: T,
    options: BaseJsonManagerOptions = {} 
  ) {
    this.filePath = path.join(app.getAppPath(), 'data', fileName);
  ...
  public async load(): Promise<void> {
    ...
    if (fileExists && this.options.backupOnLoad) {
      const timestamp = new Date().toISOString().replace(/[:.]/g, '-').replace(/[T]/g, '_').replace(/[Z]/g, '');
